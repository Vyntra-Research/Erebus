...
const environmentPath = (value: string): string => NodePath.normalize(value);
          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true && path.includes("Erebus (Alpha)"),
                  ),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("uses the isolated Erebus userData path without adopting T3 state", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(
          userDataPath,
          environmentPath("/Users/alice/Library/Application Support/erebus"),
        );
      }),
  ...
  it.effect("does not probe a legacy path when Erebus uses its own identity", () => {
    const legacyPath = environmentPath("/Users/alice/Library/Application Support/erebus");
    ...
    return withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        assert.equal(yield* identity.resolveUserDataPath, legacyPath);
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Erebus"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Erebus");
