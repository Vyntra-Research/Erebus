...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    ...
    userDataDirName: isDevelopment ? "erebus-dev" : "erebus",
    legacyUserDataDirName: isDevelopment ? "Erebus (Dev)" : "Erebus (Alpha)",
...
describe("DesktopClerk", () => {
  ...
  it.effect("acquires and releases the SDK bridge with the layer", () => {
    ...
    return Effect.gen(function* () {
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "erebus-dev", host: "app" },
          },
        ],
      ]);
      ...
      assert.deepEqual(events, ["setPath:userData:/tmp/app-data/erebus-dev", "createClerkBridge"]);
  ...
  it.each([
    { isDevelopment: true, scheme: "erebus-dev" },
    { isDevelopment: false, scheme: "erebus" },
