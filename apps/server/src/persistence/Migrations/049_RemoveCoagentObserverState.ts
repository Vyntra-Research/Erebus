...
export default Effect.gen(function* () {
  ...
  yield* sql`ALTER TABLE coagent_threads RENAME TO coagent_threads_v48`;
  yield* sql`
  ...
  yield* sql`
    INSERT INTO coagent_threads (
      child_thread_id, parent_thread_id, project_id, assignment, creation_mode,
      status, error, created_at, updated_at
    )
    SELECT
      child_thread_id, parent_thread_id, project_id, assignment, creation_mode,
      status, error, created_at, updated_at
    FROM coagent_threads_v48
  `;
  yield* sql`DROP TABLE coagent_threads_v48`;
  yield* sql`
