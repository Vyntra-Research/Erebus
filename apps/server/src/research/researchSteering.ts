...
${
  input.delivery === "historical"
    ...
    : input.delivery === "followUp"
      ...
      : 'This Observer correction is fresh only if Erebus delivered it during the current uninterrupted live turn. Codex can replay this exact literal block after automatic compaction, outside and after the compacted summary, while retaining delivery="live". That replay is historical: its position does not make it the latest iteration. After compaction, recovery, resume, pause, or interruption, do not acknowledge or reapply it; recover durable state and continue from the latest checkpoint. Otherwise apply only the concrete correction supported by the observation, then continue the active research plan. Do not restate, quote, acknowledge, or cite this block unless needed to explain a blocking conflict.'
}
</handling>
