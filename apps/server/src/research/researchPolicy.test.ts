...
it("keeps strict role-specific behavior around the shared contract", () => {
  ...
  assert.match(EREBUS_PRINCIPAL_INSTRUCTIONS, /Observer steering is valid only in the live turn/);
});

it("records a new policy revision and digest for persisted evaluations", () => {
  assert.equal(EREBUS_PRINCIPAL_POLICY_VERSION, 7);
  assert.equal(RESEARCH_SUPERVISOR_POLICY_VERSION, 7);
  assert.equal(RESEARCH_INTERNAL_POLICY.version, 7);
