...
const childAssignmentPrompt = (input: {
  ...
  [
    ...
    "The Erebus command and workspace safety policy remains binding. Never use rg or recursively traverse dependency trees or links. A recursive search of the assigned codebase or one explicit source subtree is allowed when it does not enter node_modules or package stores or follow links, junctions, or reparse points. A relevant bounded generated or compiled subtree inside the target is not automatically unsafe. The workspace is a containment boundary, not a request to create a lab. Reuse the assigned target and campaign environment; do not invent a LABS tree, fixture copy, or per-round directory unless a concrete writable test needs isolation or persistent evidence. Read-only work must not create one. Use system temp for disposable scratch data, and clean only exact task-owned artifacts and runtime resources when they stop being useful. Docker, WSL, Git, and explicitly scoped external paths remain available.",
...
const principalInstructions = (isChild: boolean) =>
  isChild
    ? [
        ...
        "- The global Erebus command and workspace safety policy applies to you and every native subagent. A recursive search of the assigned codebase or one explicit source subtree is allowed when it does not enter node_modules or package stores or follow links, junctions, or reparse points. A relevant bounded generated or compiled subtree inside the target is not automatically unsafe. The workspace is a containment boundary, not a request to create a lab. Reuse the assigned target and campaign environment; do not invent a LABS tree, fixture copy, or per-round directory unless a concrete writable test needs isolation or persistent evidence. Read-only work must not create one. Use system temp for disposable scratch data. Never use rg, recursively traverse or copy dependency trees or links, use unrelated host directories as artifact dumps, or run broad cleanup. Normal scoped Docker, WSL, Git, and external-target work is allowed.",
    ...
    : [
        ...
        "- The global Erebus command and workspace safety policy applies to the parent, each co-agent, and every native subagent. A recursive search of the assigned codebase or one explicit source subtree is allowed when it does not enter node_modules or package stores or follow links, junctions, or reparse points. A relevant bounded generated or compiled subtree inside the target is not automatically unsafe. A workspace is a containment boundary, not a request to create a lab. Reuse the assigned target and campaign environment; do not invent a LABS tree, fixture copy, or per-round directory unless a concrete writable test needs isolation or persistent evidence. Read-only work must not create one. System temp is for disposable scratch data. Clean only verified task-owned resources; normal scoped Docker, WSL, Git, and external-target work is allowed.",
