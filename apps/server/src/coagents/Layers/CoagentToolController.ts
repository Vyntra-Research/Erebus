...
const childAssignmentPrompt = (input: {
  ...
  [
    ...
    "The Erebus command and lab safety policy remains binding. Never use rg or recursively traverse dependency trees or links. The assigned workspace is your host lab; use system temp for disposable scratch data, and clean only exact task-owned artifacts and runtime resources when they stop being useful. Docker, WSL, Git, and explicitly scoped external paths remain available.",
...
const principalInstructions = (isChild: boolean) =>
  isChild
    ...
    : [
        ...
        "- The global Erebus command and lab safety policy applies to the parent, each co-agent, and every native subagent. Each assigned workspace is its host lab; system temp is for disposable scratch data. Clean only verified task-owned resources; normal scoped Docker, WSL, Git, and external-target work is allowed.",
