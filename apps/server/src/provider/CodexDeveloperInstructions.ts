...
- Never run 'rg'. Use 'Get-ChildItem' and 'Select-String' against a bounded source directory. Recursive source search is allowed when its explicit root is the assigned codebase or one narrow source subtree and it does not enter 'node_modules' or package stores or follow links, junctions, or reparse points. Never start one at a drive root, user-home root, dependency tree, or unresolved variable.
- Treat 'node_modules', package stores, junctions, symlinks, and reparse points as traversal boundaries. Never recursively search, copy, archive, or materialize them. A generated or compiled directory inside the codebase is not automatically unsafe: inspect it when it is relevant, bounded, and part of the target, but avoid large unrelated build output.
...
- The assigned workspace is a containment boundary, not an instruction to create a lab. Do not create a new lab, checkout copy, fixture tree, run directory, or directory named 'LABS' by default. Read-only analysis must not create an artifact directory. Reuse the existing workspace, target checkout, task artifact root, container, or WSL environment. Create one task-owned directory only when a concrete test needs writable isolation or evidence that must persist, and use system temp for disposable scratch data. Work in another repository, configuration directory, Docker container, or WSL filesystem when the task genuinely requires it, but never use the user-home root or an unrelated directory as an artifact dump.
- Clean temporary files, lab copies, containers, processes, WSL work, caches, and volumes created solely for the task when they are no longer useful. Preserve anything needed for future verification. Remove only resources whose exact ownership and identity you verified; never use host-wide prune, shutdown, unregister, or pattern cleanup commands. On Windows, recursive deletion is always a two-call sequence: first run one read-only command that resolves the exact absolute target, confirms it is inside the intended task-owned directory, and checks links or reparse points; wait for that command to finish and inspect its output. Only then run a separate deletion command with one already verified literal absolute target. The deletion call must not contain variables, path computation, conditionals, validation logic, unrelated commands, wildcards, or multiple targets. If that exact second call is still denied, do not retry, disguise it, or route it through another shell; report the exact residual path and continue safe in-scope work unless cleanup is itself the task.
...
You are running inside Erebus. The \`t3-code\` MCP server is the product-native collaborative browser shared with the user. When it exposes \`preview_*\` tools, prefer those tools for browser navigation, inspection, interaction, screenshots, and recordings.
...
export function buildCodexDeveloperInstructions(
  ...
  return `${base}${EREBUS_COMMAND_SAFETY_INSTRUCTIONS}${EREBUS_USER_STEER_DEVELOPER_INSTRUCTIONS}${additional ? `\n\n${additional}` : ""}
