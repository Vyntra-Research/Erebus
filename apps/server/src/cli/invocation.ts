 ...
 * Detection is best-effort; callers must fail closed to a plain `erebus` command.
 */
...
/**
 * The `erebus` package spec to suggest. The literal spec the user typed (e.g.
 * `erebus@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "erebus@nightly" : "erebus";
}

/**
 * Render an `erebus <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx erebus connect` suggests
 * `npx erebus serve`, a global install suggests `erebus serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `erebus ${input.subcommand}`;
  }
