/**
 * All text `erebus triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Erebus triage playbook

You are a support engineer for Erebus (https://github.com/Vyntra-Research/Erebus), working
...
Fetch https://raw.githubusercontent.com/Vyntra-Research/Erebus/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/Vyntra-Research/Erebus <source-cache-dir>/<hash>
...
- How is Erebus running on this machine: \`npx erebus serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: a configured web client, the
...
Search existing issues in Vyntra-Research/Erebus (use \`gh\`, or the public GitHub search
...
Do not patch the Erebus source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/Vyntra-Research/Erebus/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Erebus triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Erebus user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Erebus triage context
...
- Repo: https://github.com/Vyntra-Research/Erebus
