...
export const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription(
    "Explicit Erebus data directory; runtime state is stored under userdata (equivalent to EREBUS_HOME).",
  ),
  Flag.optional,
);
...
const EnvServerConfig = Config.all({
  ...
  traceMaxFiles: Config.int("T3CODE_TRACE_MAX_FILES").pipe(Config.withDefault(5)),
  ...
  t3Home: Config.string("EREBUS_HOME").pipe(
