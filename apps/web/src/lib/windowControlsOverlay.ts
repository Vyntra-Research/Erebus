...
const ELECTRON_TRANSLUCENT_CLASS_NAME = "electron-translucent";
...
export function getElectronPlatformClassNames(platform: string): readonly string[] {
  if (isWindowsPlatform(platform)) {
    return [ELECTRON_CLASS_NAME, ELECTRON_WINDOWS_CLASS_NAME, ELECTRON_TRANSLUCENT_CLASS_NAME];
  }
  if (isLinuxPlatform(platform)) {
    return [ELECTRON_CLASS_NAME, ELECTRON_TRANSLUCENT_CLASS_NAME];
  }
