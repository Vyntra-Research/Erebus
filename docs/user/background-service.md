...
> Inherited T3 Code documentation. The Linux background service is not enabled in Erebus `0.1.5`.
...
Install it with the latest Erebus release:
...
Updating restarts Erebus briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The service runs a small stable launcher. Exact Erebus versions are installed separately, so a
...
want Erebus to start in the background.
