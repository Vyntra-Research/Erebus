...
An update keeps projects, tasks, settings, native goals, and stored Judge reviews. Older campaign records remain readable for compatibility, but current Erebus releases do not resume or change them. If the built-in updater cannot complete, download the newer Windows installer or Linux AppImage from [Erebus releases](https://github.com/Vyntra-Research/Erebus/releases).

Linux auto-update is available when Erebus runs from its AppImage. Keep the AppImage in a user-writable location so Erebus can replace it after downloading an update. A package-manager installation should be updated through that package manager instead.
...
The inherited hosted web app, mobile client, and relay updater are not enabled in the current release.
