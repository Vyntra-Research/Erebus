...
This guide is for people who want to use more than one Claude setup in Erebus. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Erebus](./install.md).
...
In Erebus Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Erebus uses Claude Code's normal config directory.

When you set this field, Erebus points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Erebus
...
Erebus looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Erebus looks.

Then add another Claude provider in Erebus:
...
Erebus only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Erebus keeps separate config directories isolated
...
Add or edit a Claude provider in Erebus Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Erebus stores the value as a server secret and does not
...
Erebus does not need a special Claude Code Router provider. Treat the router as a Claude
