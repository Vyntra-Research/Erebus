...
A provider is the agent runtime that does the actual work. The current Erebus release enables Codex only. The
...
Only `codex` enters the active registry in the current release. Erebus does not show, probe, refresh, or update
